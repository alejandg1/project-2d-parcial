...
function setRelationValue(autores,collaborations){
  let x = 0;
  let links = getAuthorRelations(autores);
          links.forEach((link) => {
            if (!collaborations.links.includes(link)) {
              if (x == 0) {
                collaborations.links.push(link);
              }
              collaborations.links.forEach((existingLink) => {
                if (
  ...
  $.ajax({
    ...
    success: (data) => {
      ...
      data.forEach((publicacion) => {
        ...
        // if (autores.length > 1) {
          setRelationValue(autores,collaborations);
        //}
      });git sta
